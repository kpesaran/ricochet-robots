...
export default class Wall {
    ...
    setGeometry() {
        this.geometry = new THREE.BoxGeometry(1, 0.5, 0.1);
    }
    ...
    setMesh(direction: Direction, pos: Position) {
        ...
        if (direction === Direction.North) {
            this.mesh.position.set(pos.column - 7.5, 0.25, pos.row - 8);
            this.mesh.rotation.x = Math.PI ;
        }
        if (direction === Direction.South) {
            this.mesh.position.set(pos.column - 7.5, 0.25, pos.row - 7);
            this.mesh.rotation.x = Math.PI ;
        }
        if (direction === Direction.West) {
            this.mesh.position.set(pos.column - 8, 0.25, pos.row - 7.5);
            this.mesh.rotation.y = Math.PI * 0.5;
        }
        if (direction === Direction.East) {
            this.mesh.position.set(pos.column - 7, 0.25, pos.row - 7.5);
            this.mesh.rotation.y = Math.PI * 0.5;
        }
}
    }
